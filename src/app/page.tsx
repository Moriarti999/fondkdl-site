...
async function getHomeCampaigns() {
  try {
    ...
    // 🔥 ПРИНУДИТЕЛЬНАЯ ОЧИСТКА: только примитивы
    return all
      .filter(item => String(item.entry?.status || 'active') === 'active')
      .map(item => {
        const e = item.entry || {};
        return {
    ...
    <main className="min-h-screen bg-white text-gray-800">
      ...
      <section className="relative bg-gradient-to-br from-orange-600 via-orange-700 to-orange-800 text-white py-20 px-4">
        <div className="max-w-5xl mx-auto text-center relative z-10">
          ...
          {/* ✅ Простые кнопки вместо HeroButtons */}
          <div className="flex flex-col md:flex-row gap-4 justify-center">
            <a href="/donate" className="px-6 py-3 bg-white text-orange-700 rounded-xl font-bold hover:scale-105 transition">
              💰 Помочь деньгами
            </a>
            <a href="/volunteer" className="px-6 py-3 bg-orange-700 text-white rounded-xl font-bold hover:bg-orange-800 transition">
              🤝 Помочь делом
            </a>
          </div>
        </div>
