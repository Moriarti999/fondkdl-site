      ...
      <FadeIn>
        <section className="bg-gradient-to-br from-orange-600 via-orange-700 to-orange-800 text-white py-20 px-4 text-center">
          ...
          <p className="text-lg opacity-95 max-w-2xl mx-auto">
            Ваше время и усилия — бесценный вклад в добрые дела
          </p>
        </section>
      </FadeIn>
      ...
      <FadeIn delay={0.2}>
        <section className="py-16 px-4 bg-white">
          <div className="max-w-5xl mx-auto">
            <h2 className="font-heading text-3xl font-bold text-orange-800 mb-4 text-center">
              Актуальные задачи
            </h2>
            <p className="text-center text-gray-600 mb-12 max-w-2xl mx-auto">
              Выберите дело по душе — каждая помощь важна
            </p>
            ...
            {tasks.length === 0 ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {tasks.map(({ slug, entry }: any) => (
                  <article
                    key={slug}
                    className="bg-orange-50 rounded-xl p-6 border border-orange-100 hover:shadow-lg hover:border-orange-300 transition-all"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <h3 className="font-bold text-lg text-gray-900">{entry.title}</h3>
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        entry.category === 'events' ? 'bg-purple-100 text-purple-700' :
                        entry.category === 'items' ? 'bg-green-100 text-green-700' :
                        entry.category === 'transport' ? 'bg-blue-100 text-blue-700' :
                        entry.category === 'online' ? 'bg-gray-100 text-gray-700' :
                        'bg-orange-100 text-orange-700'
                      }`}>
                        {entry.category === 'events' ? '👥 Мероприятия' :
                         entry.category === 'items' ? '📦 Вещи' :
                         entry.category === 'transport' ? '🚗 Транспорт' :
                         entry.category === 'online' ? '💻 Онлайн' :
                         '🏠 Дом'}
                      </span>
                    </div>
                    ...
                    <div className="space-y-2 text-sm text-gray-700 mb-4">
                      {entry.timeRequired && (
                        <p>⏱️ <span className="text-gray-500">Время:</span> {entry.timeRequired}</p>
                      )}
                      {entry.location && (
                        <p>📍 <span className="text-gray-500">Место:</span> {entry.location}</p>
                      )}
                      {entry.contact && (
                        <p>📞 <span className="text-gray-500">Контакт:</span> {entry.contact}</p>
                      )}
                    </div>
                    
                    <a
                      href={`https://wa.me/79287312602?text=Здравствуйте!%20Хочу%20помочь%20с%20задачей:%20${encodeURIComponent(entry.title)}`}
                      target="_blank"
        ...
        <section className="py-16 px-4 bg-orange-50 text-center">
          <div className="max-w-2xl mx-auto">
            <h2 className="font-heading text-2xl md:text-3xl font-bold text-orange-800 mb-4">
              Не нашли задачу по душе?
            </h2>
            <p className="text-gray-600 mb-6">
              Напишите нам — мы подберём дело, которое подойдёт именно вам
            </p>
            <a
              href="https://wa.me/79287312602"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block px-8 py-3 bg-orange-600 text-white rounded-xl font-semibold hover:bg-orange-700 transition-all shadow-md"
            >
              Написать в WhatsApp 💬
            </a>
          </div>
